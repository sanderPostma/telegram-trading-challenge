...
pub fn scale_order(size: Size, input: ScaleInput) -> anyhow::Result<ScaledOrder> {
    ...
    let qty = round_down(raw_qty, input.qty_step);
    Ok(ScaledOrder {
        ratio,
        qty,
...
pub fn scale_close(position_qty: f64, pct: f64, qty_step: f64) -> anyhow::Result<f64> {
    anyhow::ensure!(position_qty >= 0.0, "position qty cannot be negative");
    anyhow::ensure!((0.0..=1.0).contains(&pct), "close pct must be 0..1");
    Ok(round_down(position_qty * pct, qty_step))
}
...
mod tests {
    ...
    fn calc_dart_golden_usd_position() {
        ...
        let scaled = scale_order(Size::Usdt(31_000.0), input).unwrap();
    ...
    fn btc_size_scales_by_balance_ratio() {
        ...
        let scaled = scale_order(Size::Coin(0.5), input).unwrap();
        assert_eq!(scaled.qty, 0.01);
